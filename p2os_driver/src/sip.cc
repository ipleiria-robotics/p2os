...
void SIP::FillStandard(ros_p2os_data_t *data) {
    ...
    data->position.pose.pose.orientation = createQuaternionMsgFromYaw(pa);


    // add rates
    data->position.twist.twist.linear.x = ((lvel + rvel) / 2) / 1e3;
    data->position.twist.twist.linear.y = 0.0;
    data->position.twist.twist.angular.z = ((double) (rvel - lvel) /
    ...
    data->odom_trans.transform.rotation = createQuaternionMsgFromYaw(pa);
